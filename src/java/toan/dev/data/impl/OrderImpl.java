...
public class OrderImpl implements OrderDao {
    ...
    public Order findByCode(String code) {
        System.out.println("Tìm kiếm đơn hàng với mã code: " + code); // In mã code cần tìm
        ...
        try {
            ...
            if (rs.next()) { // Sửa đổi `while` thành `if` vì chỉ cần lấy một dòng kết quả
                int id = rs.getInt("id");
                String status = rs.getString("status");
                int userId = rs.getInt("user_id");
                Timestamp createdAt = rs.getTimestamp("created_at");
                ...
                System.out.println("Đã tìm thấy đơn hàng: " + order); // In ra đơn hàng tìm thấy
                return order;
            } else {
                System.out.println("Không tìm thấy đơn hàng với mã code: " + code); // In thông báo nếu không tìm thấy
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
